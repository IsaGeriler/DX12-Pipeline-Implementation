...
{
	tr.x = std::min<float>(std::max<float>(std::max<float>(v0.x, v1.x), v2.x), canvas.getWidth() - 1 / 1.f);
	tr.y = std::min<float>(std::max<float>(std::max<float>(v0.y, v1.y), v2.y), canvas.getHeight() - 1 / 1.f);
